...
// Struct for command options
typedef struct {
    int show_all;    // -a: Show all files including hidden
    int long_list;   // -l: Long listing format
    int recursive;   // -R: Recursively list subdirectories
    int sort_time;   // -t: Sort by modification time
    int reverse;     // -r: Reverse the order of sort
    int show_inode;  // -i: Display inode number
    int show_blocks; // -s: Display block size
} Options;

// Linked list node for files
...
FileNode *add_file(FileNode *head, const char *name, const struct stat *statbuf);
...
int main(int argc, char *argv[]) {
    Options opts = {0}; // Initialize all options to 0
    char *target_dir = ".";
...
void sort_files(FileNode **head, const Options *opts) {
    ...
    while (*head) {
        ...
        *head = (*head)->next;
        ...
        if (!sorted || (opts->sort_time
                            ? curr->statbuf.st_mtime > sorted->statbuf.st_mtime
                            : strcmp(curr->name, sorted->name) < 0)) {
            curr->next = sorted;
            sorted = curr;
        } else {
            FileNode *s = sorted;
            while (s->next && (opts->sort_time
...
void print_file(const FileNode *file, const Options *opts) {
    ...
    if (opts->long_list) {
        char perms[11] = "----------";
        if (S_ISDIR(file->statbuf.st_mode)) perms[0] = 'd';
        if (S_ISLNK(file->statbuf.st_mode)) perms[0] = 'l';
        if (file->statbuf.st_mode & S_IRUSR) perms[1] = 'r';
        if (file->statbuf.st_mode & S_IWUSR) perms[2] = 'w';
        if (file->statbuf.st_mode & S_IXUSR) perms[3] = 'x';
        if (file->statbuf.st_mode & S_IRGRP) perms[4] = 'r';
        if (file->statbuf.st_mode & S_IWGRP) perms[5] = 'w';
        if (file->statbuf.st_mode & S_IXGRP) perms[6] = 'x';
        if (file->statbuf.st_mode & S_IROTH) perms[7] = 'r';
        if (file->statbuf.st_mode & S_IWOTH) perms[8] = 'w';
        if (file->statbuf.st_mode & S_IXOTH) perms[9] = 'x';

        struct passwd *pw = getpwuid(file->statbuf.st_uid);
        struct group *gr = getgrgid(file->statbuf.st_gid);
        char time_str[20];
        strftime(time_str, sizeof(time_str), "%b %d %H:%M", localtime(&file->statbuf.st_mtime));
