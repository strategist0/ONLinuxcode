...
void sort_files(FileNode **head, const Options *opts)
{
    ...
    while(*head){
        ...
        *head = (*head)->next;
        ...
        if(!prev || opts->sort_time
                                   ? cur->statbuf.st_mtime >prev->statbuf.st_mtime
...
void print_file(const FileNode *file, const Options *opts)
{
    ...
    if(opts->long_list){
        ...
        char timestr[26];
        strftime(timestr, sizeof(timestr), "%b %d %H:%M", localtime(&file->statbuf.st_mtime));
